...
const Create = inject(Stores.MapStore)(
  observer((props: IProps) => {
    const { editMap, events } = props.mapStore;
    useEffect(() => {
      initValue();
      return () => props.mapStore.cleanUp();
    }, []);
    ...
    const onFinish = async (value: any) => {
      ...
      try {
        var result = await props.mapStore.create(editMap);
        console.log("created: ", result);

        toast("Create success!");
      } catch (error) {
        toast("Faild creation!");
      }
      setTimeout(() => props.navigate(AppLayouts.homePage.path), 1000);
    };
    ...
    const SaveAndAdd = () => {
      return (
        <Row style={{ flexWrap: "nowrap", gap: "30px" }}>
          <Col className="flex justify-content-center items-center">
            <PlusCircleOutlined
              style={{ fontSize: "25px" }}
              onClick={() => {
          ...
          <Col className="flex justify-content-center items-center">
            <Button
              ...
              onClick={() => {
                ...
                document.getElementById("form-info-map")?.click();
              }}
            >
              Create
            </Button>
